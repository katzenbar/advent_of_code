...
type Grid = {
  [coordinates: string]: Set<number> | undefined;
};
...
const traceWire = (
  ...
  for (let i = 0; i < pathSegment[1]; i++) {
    ...
    const gridValue = currentGrid[key] || new Set();
    gridValue.add(wireKey);

    currentGrid[key] = gridValue;
  }

  return traceWire(wireKey, currentGrid, currentX, currentY, remainingPath);
};

const distancesToCrossings = (grid: Grid): Array<number> => {
  return Object.entries(grid)
    .filter(([key, value]) => value && value.size > 1)
    .map(([key]) => {
...
export const solve = (input: string): number => {
  ...
  const gridAfter1 = traceWire(1, {}, 0, 0, wire1);
  const finalGrid = traceWire(2, gridAfter1, 0, 0, wire2);
